...
async def create_area(request: Request):
    ...
    if not area_name or not coordinates:
        raise HTTPException(
            status_code=400, detail="Area name and coordinates are required")
    ...
    try:
        with conn.cursor() as cur:
            try:
                ...
                if result is None:
                    raise HTTPException(
                        status_code=500, detail="Failed to insert area")
                area_id = result[0]
                conn.commit()
...
async def create_detection_log(request: Request):
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id FROM area_configurations WHERE id = %s", (area_id,))
            area_exists = cur.fetchone()

            if not area_exists:
                raise HTTPException(
                    status_code=400, detail=f"Area with ID {area_id} does not exist.")
...
def get_live_stats():
    with get_db_connection() as conn, conn.cursor() as cur:
        ...
        # Correct way to compare datetime in SQL:
        cur.execute(
            "SELECT COUNT(DISTINCT vehicle_id) "
            "FROM vehicle_logs "
            "WHERE timestamp >= %s AND status = 'entered'",
            (one_second_ago,)  # Pass the datetime object as a parameter
        )
